...
router.get('/api/modelos', async (req, res) => {
  try {
    ...
    const { count, rows } = await db.modelos.findAndCountAll({
        attributes: [
          "IdModelo",
          "Nombre",
          "Anio",
...
router.get('/api/modelos/:IdModelo', async (req, res) => {
  try {
    const modelo = await db.modelos.findByPk(req.params.id);
    if (modelo) {
      res.json(modelo);
    } else {
...
router.post('/api/modelos', async (req, res) => {
  try {
    const nuevomodelo = await db.modelos.create(req.body);
...
router.put('/api/modelos/:IdModelo', async (req, res) => {
  try {
    const [numFilasActualizadas, modeloActualizado] = await db.modelos.update(req.body, {
...
router.delete('/api/modelos/:IdModelo', async (req, res) => {
  try {
    const numFilasEliminadas = await db.modelos.destroy({
      where: { IdModelo: req.params.id },
    });
